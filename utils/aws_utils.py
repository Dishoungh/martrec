...
def create_bucket(bucket_name):
    try:
        subprocess.run(['aws', 's3api', 'create-bucket', '--bucket', bucket_name, '--region', 'us-east-1'], stdout=subprocess.DEVNULL, shell=True)
        print("[SUCCESS] AWS Bucket Created as {bn}".format(bn=bucket_name))
    except:
        try:
            subprocess.run(['aws', 's3api', 'create-bucket', '--bucket', bucket_name, '--region', 'us-east-2'], stdout=subprocess.DEVNULL, shell=True)
            print("[SUCCESS] AWS Bucket Created as {bn}".format(bn=bucket_name))
...
def configure_aws():
    ...
    if ('Linux' in platform.platform()):
        process = subprocess.Popen(['aws', 'configure'], stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, shell=True)
...
def send_to_s3(output_path, bucket_name):
    # Upload all files in output_path
    try:
        subprocess.run(['aws', 's3', 'sync', output_path, 's3://{bn}'.format(bn=bucket_name),
                        '--acl', 'public-read'], shell=True)
...
def get_csv(csv_save, bucket_name):
    try:
        ...
        objects = subprocess.run(["aws", "s3api", "list-objects", "--bucket", bucket_name,
                        ...
                        stdout=subprocess.PIPE, shell=True).stdout.splitlines()
...
def set_cors(bucket_name, cors_file):
    try:
        subprocess.run(['aws', 's3api', 'put-bucket-cors', '--bucket', bucket_name, '--cors-configuration', 'file://{c}'.format(c=cors_file)], stdout=subprocess.DEVNULL, shell=True)
